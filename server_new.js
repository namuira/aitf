...
// 에이전트별 대시보드
app.get('/agent/:id/dashboard', createAuthMiddleware(':id'), async (req, res) => {
  const agentId = req.params.id;
...
// 에이전트별 버튼 추가
app.post('/api/agent/:id/buttons', createAuthMiddleware(':id'), async (req, res) => {
  const agentId = req.params.id;
...
// 에이전트별 버튼 삭제
app.delete('/api/agent/:id/buttons/:buttonId', createAuthMiddleware(':id'), async (req, res) => {
  const agentId = req.params.id;
